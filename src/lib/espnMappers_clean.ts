...
function computeCombinedStats(
  ...
  _awaySchedule: ESPNTeamSchedule | null,
): { bttsPct: number | null; over25Pct: number | null } {
  // Use home team's schedule as the event list; each completed event has both teams' scores
  const events = homeSchedule?.events ?? []
  const bttsArr:  boolean[] = []
  const over25Arr: boolean[] = []
  ...
  for (const ev of events) {
    ...
    bttsArr.push(scores[0] > 0 && scores[1] > 0)
    over25Arr.push(total > 2.5)
  }

  return {
    bttsPct:  pct(bttsArr),
    over25Pct: pct(over25Arr),
  }
}
